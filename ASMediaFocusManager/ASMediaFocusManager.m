...
//        NSError *error = nil;
        ...
        [viewController.mainImageView setImageWithURL:url];
...
//                         focusViewController.view.backgroundColor = [UIColor greenColor];
                     }
...
+(CGRect)calculateFullScreenRectForImage:(UIImage *)image
{
    CGSize size = image.size;
    CGFloat ratio = [UIScreen mainScreen].bounds.size.width/size.width;
    CGSize newSize = CGSizeMake(size.width * ratio, size.height * ratio);
    CGRect rect;
    rect.size = newSize;
    CGFloat height = [UIScreen mainScreen].bounds.size.height;
    CGFloat origingY = (height - newSize.height)/2;
    rect.origin = CGPointMake(0, origingY);
    return rect;
}
